...
public class Machine {
    ...
    private List<Point> block = new ArrayList<Point>();   /* current "must block" list */
    ...
    public Point move() {
        ...
        if (maxWeight == MINWEIGHT && block.size() == 0) { // no feasible strategy found
        	if ((pt = blockMove()) == null) {	// try a blocking move
        		pt = randomMove();				// try a random move
        	}
        	
        	if (pt != null) board.mark(pt.x, pt.y, Graph.VT_X);
        ...
        if (block.size() > 0 && maxWeight < MAXWEIGHT) pt = block.get(0);
    ...
    Point blockMove() {
    	...
    	for (Point[] v : VECTORS) {
    		...
    		for (int i = 0; i < v.length; i++) {
                ...
                if (type == Graph.VT_O) weight += PIECEWEIGHT;
                if (type == Graph.VT_X) { weight = MINWEIGHT; break; }	// not feasible
    		}            
    		if (weight > maxWeight) {
    			maxWeight = weight;
    			maxV = v;
    	...
    	if (maxV != null) {
    		for (int i = 0; i < maxV.length; i++) {
    			int type = board.markType(maxV[i].x, maxV[i].y);
    			if (type == Graph.VT_E) return maxV[i];
    		}    		
    	}
    ...
    Point randomMove() {
        ...
        if (empty.size() > 0) {
            int n = (int)Math.round(Math.random() * (empty.size()-1));
            return empty.get(n);
        }
    ...
    private boolean center(Point[] v) {
        for (Point aV : v) {
            int square = aV.x * Board.SIZE + aV.y;
    ...
    private Point getSquare(Point[] v) {
        ...
        // first try center
        if (center(v)) return new Point(CENTER / Board.SIZE, CENTER % Board.SIZE);
        ...
        for (int i = 0; i < v.length; i++) {
            if (board.markType(v[i].x, v[i].y) == Graph.VT_X) {
                if (i > 0 && board.markType(v[i-1].x, v[i-1].y) == Graph.VT_E) {
                    weight = weight(v[i-1]);
                    if (weight > maxWeight) {
                        maxWeight = weight;
                        maxP = v[i-1];
                ...
                if (i < v.length - 1 && board.markType(v[i+1].x, v[i+1].y) == Graph.VT_E) {
                    weight = weight(v[i+1]);
                    if (weight > maxWeight) {
                        maxWeight = weight;
                        maxP = v[i+1];
    ...
    private float weight(Point p) {
        ...
        Iterator<Point[]> it = VECTORS.iterator();
        while(it.hasNext()) {
            Point[] v = it.next();
            for (i = 0; i < v.length; i++) {
                if (v[i] == p) break;   // contained
            }

            if (i < v.length) {
                vweight = weight(v);
                if (vweight > MINWEIGHT) // contained and feasible
                    weight += vweight;
            }
        }
    ...
    private float weight(Point[] v) {
        ...
        for (Point aV: v) {
            int type = board.markType(aV.x, aV.y);
            switch (type) {
            case Graph.VT_O: return MINWEIGHT;  // vector must contain no O's
            case Graph.VT_X: weight += PIECEWEIGHT; break;
            }
        }
        ...
        if (block.size() > 0) {
            int bcount = 0;
            for (int i = 0; i < v.length; i++) {
                if (block.contains(v[i])) bcount++;
            }
            if (bcount == 0) return MINWEIGHT;  // no blocking strategy
        }
        ...
        int m = v[MEANPOINT].x, n = v[MEANPOINT].y;
        int x = CENTER / Board.SIZE, y = CENTER % Board.SIZE;
        float d = Math.abs(x - m) + Math.abs(y - n) + 1;
        weight += 1/d;
    ...
    /*
     * Determine squares where we must block
     * where the vector contains Board.SIZE - 1 O's and one empty
     */
    private void getBlock() {
        ...
        while(it.hasNext()) {
            ...
            for (int i = 0; i < v.length; i++) {
                ...
                if (type == Graph.VT_E) { ecount++; b = v[i]; }
            }

            if (ocount == Board.VSIZE - 1 && ecount == 1) { // must block, b contains the square to block
                weight = weight(b);
                if (weight > maxWeight) {
                    maxP = b;
                    maxWeight = weight;
