...
public class View extends JComponent implements MouseListener {
    ...
    public static final int CELL_CX = 75;
    public static final int CELL_CY = 75;
    ...
    private TicTacToe theGame = TicTacToe.getInstance();
    ...
    private void paintBoard(Graphics2D g) {
        ...
        for (int i = bx; i < cx; i++) {
            for (int j = by; j < cy; j++) {
                ...
                g.drawImage(image, null, BORDER_CX + i * CELL_CX, BORDER_CY + j * CELL_CY);
            }
        }
    }
	...
	public void mouseClicked(MouseEvent e) {
        ...
        if (p.contains(pt) && px < cx && py < cy) {
            int color = board.getPiece(px, py);
            if (color == Board.COLOR_EMPTY) {
                ...
                if (pt != null) {
                    p = polygonFromPoint(new Point(BORDER_CX + pt.x * CELL_CX, BORDER_CY + pt.y * CELL_CY));
                    rc = p.getBounds();
                    repaint(rc);
                }
            }
