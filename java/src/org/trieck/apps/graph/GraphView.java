...
public class GraphView extends JComponent implements MouseListener {
    ...
    private static final Polygon HEXAGON = new Polygon(XPOINTS, YPOINTS, XPOINTS.length);
    ...
    private static class Fill {
        private Point pt;       // polygon index
        private int color;      // palette index
		...
		public boolean equals(Object o) {
            ...
            if (color != fill.color) return false;
            if (pt != null ? !pt.equals(fill.pt) : fill.pt != null) return false;
    ...
    private void init() {
        ...
        setPreferredSize(new Dimension(CELL_CX * CX_POLYGONS +
                BORDER_CX * 2,
                CELL_CY * CY_POLYGONS +
                BORDER_CY * 2));
    ...
    private void createBuffers() {
        ...
        // non-filled image
        buff[0] = new BufferedImage(CELL_CX + 1, CELL_CY + 1, BufferedImage.TYPE_INT_ARGB);
        Graphics2D gbi = buff[0].createGraphics();
        gbi.setColor(Color.WHITE);
        gbi.drawPolygon(HEXAGON);

        // filled image color 1
        buff[1] = new BufferedImage(CELL_CX + 1, CELL_CY + 1, BufferedImage.TYPE_INT_ARGB);
        ...
        buff[2] = new BufferedImage(CELL_CX + 1, CELL_CY + 1, BufferedImage.TYPE_INT_ARGB);
    ...
    private void paintGraph(Graphics2D g) {
        ...
        int cx = (rc.x + rc.width) / CELL_CX;
        int cy = (rc.y + rc.height) / CELL_CY;
        ...
        for (int i = by; i <= cy; i++) {
            for (int j = bx; j <= cx; j++) {
                ...
                if (f != null) {
                    if (f.color == 0)
                        g.drawImage(buff[1], null, BORDER_CX + j * CELL_CX, BORDER_CY + i * CELL_CY);
                    else
                        g.drawImage(buff[2], null, BORDER_CX + j * CELL_CX, BORDER_CY + i * CELL_CY);
                } else {
                    g.drawImage(buff[0], null, BORDER_CX + j * CELL_CX, BORDER_CY + i * CELL_CY);
                }
            }
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        int cx = width / CELL_CX;
        int cy = height / CELL_CY;

        int px = (pt.x - BORDER_CX) / CELL_CX;
        int py = (pt.y - BORDER_CY) / CELL_CY;

        Polygon p = polygonFromPoint(new Point(BORDER_CX + px * CELL_CX, BORDER_CY + py * CELL_CY));

        String str = "";
        if (p.contains(pt) && px < cx && py < cy) {
            str = py + ", " + px;
            Point ptf = new Point(px, py);
            Fill f = getFill(ptf);
            if (f != null) {
                filled.remove(f);
            } else {
        ...
        GraphFrame frame = app.getMainFrame();
