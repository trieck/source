...
NSTR[18]:='NEQ       '; INSTR[19]:='GEQ       ';
   ...
   BEGIN  
      WHILE NOT EOLN(PRD) DO
            ...
            END 
   END; (*GENERATE*)
   PROCEDURE ASSEMBLE; (*TRANSLATE SYMBOLIC CODE INTO MACHINE CODE AND
 STORE*)
         ...
         CASE Q OF
              ...
              3 (*RLN*) : BEGIN CASE STORE[SP].VA OF
               ...
               10 (*WRC*):BEGIN CASE STORE[SP].VA OF
                                     4: ERRORI(' WRITE ON INPUT FILE     ');
                                     5: WRITE(OUTPUT,CHR(STO
RE[SP-2].VI):STORE[SP-1].VI); 
                                     6: ERRORI(' WRITE ON PRD FILE       ');
...
BEGIN  CASE OP OF (* IN THIS PROCEDURE Q MUST NOT BE CORRECTED *) 
          ...
          21 (*LEQ*):BEGIN  SP := SP-1; 
                      ...
                      0,1: STORE[SP].VB := STORE[SP].VI<=STORE[SP+1].VI; 
                        ...
                        4: STORE[SP].VB := STORE[SP].VS<=STORE[SP+1].VS; 
                        5: BEGIN  COMPARE;
                              STORE[SP].VB := (STORE[I1+I].V
I<=STORE[I2+I].VI)OR B 
                           END; 
                      END; (*CASE P*) 
                      STORE[SP].STYPE := BOOL 
                   END; 
