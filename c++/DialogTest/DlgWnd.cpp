...
void CDlgWin::CreateControls()
{
    ...
    // Create them
    m_pOpenBtn->Create(L"&Open File", BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD,
                       CRect(350, 8, 513, 33), this, ID_OPENBTN);
    m_pExitBtn->Create(L"E&xit", BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD,
    ...
    m_pListBox->Dir(DDL_READONLY, L"*.ini");
...
void CDlgWin::OnBtnOpenClick()
{
    ...
    if ((nIndex = m_pListBox->GetCurSel())!=-1) {
        ...
        m_pListBox->GetText(nIndex, szFile);

        // create file
        m_pFile = new CStdioFile(szFile, CFile::modeRead | CFile::typeBinary);
        ...
        if (!(lpBuff)) {
            AfxMessageBox(L"Error allocating memory.", MB_ICONINFORMATION);
            return;
        }
...
void CDlgWin::SetWndFont(CWnd* pWnd, CString szFont, LONG lSize)
{
    if (!m_pFont) {
        ...
        lstrcpy(m_lf.lfFaceName, szFont);
