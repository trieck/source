...
/////////////////////////////////////////////////////////////////////////////
SHA1::SHA1(const string &s)
{
	digest[0] = '\0';
	sha1_ctx ctx;
	sha1_init(&ctx);
	sha1_update(&ctx, (const unsigned char *)s.c_str(), s.length());
	sha1_final(&ctx, (unsigned char *)digest);
}

/////////////////////////////////////////////////////////////////////////////
SHA1::SHA1(const void *pv, unsigned len)
{
	digest[0] = '\0';
	sha1_ctx ctx;
	sha1_init(&ctx);
	sha1_update(&ctx, (const unsigned char *)pv, len);
	sha1_final(&ctx, (unsigned char*)digest);
}
